...
// --- डेटा ---
// डेटा को कंपोनेंट के बाहर रखने से यह साफ़ रहता है।
const templatesData = [
    {
        id: 1,
        name: 'Classic Floral Invitation',
        previewImages: ['https://images.unsplash.com/photo-1593563919199-35f6f477a3ce?q=80&w=1887&auto=format&fit=crop'],
        downloadFormats: { PSD: '#', JPG: '#', PNG: '#' },
        category: 'Invitations',
        aiPrompt: `Draft a formal wedding invitation for [FEMALE_NAME] and [MALE_NAME], who are getting married on [WEDDING_DATE] at [ADDRESS]. Request guests' presence to celebrate their union. Ensure the tone is elegant and traditional.`
    },
    {
        id: 2,
        name: 'Modern Geometric Save-the-Date',
        previewImages: ['https://images.unsplash.com/photo-1560962827-2a68f4e20557?q=80&w=1887&auto=format&fit=crop'],
        downloadFormats: { EPS: '#', PNG: '#' },
        category: 'Save-the-Date',
        aiPrompt: `Create a concise and modern save-the-date message for [FEMALE_NAME] and [MALE_NAME]'s wedding. The date is [WEDDING_DATE]. Mention that formal invitations will follow. Keep it short and chic.`
    },
    {
        id: 3,
        name: 'Elegant Thank You Card',
        previewImages: ['https://images.unsplash.com/photo-1558902804-f7de5d85c490?q=80&w=1887&auto=format&fit=crop'],
        downloadFormats: { PSD: '#', JPG: '#' },
        category: 'Thank You Cards',
        aiPrompt: `Write a heartfelt thank you note from [FEMALE_NAME] and [MALE_NAME] for gifts received after their wedding on [WEDDING_DATE]. Express gratitude sincerely.`
    },
    {
        id: 4,
        name: 'Rustic Wedding Program',
        previewImages: ['https://images.unsplash.com/photo-1549923289-518f8bff3c3d?q=80&w=1887&auto=format&fit=crop'],
        downloadFormats: { PSD: '#', EPS: '#', PNG: '#' },
        category: 'Programs',
        aiPrompt: `Generate a wedding program outline for the marriage of [FEMALE_NAME] and [MALE_NAME] on [WEDDING_DATE] at [ADDRESS], including ceremony events (e.g., Processional, Vows, Recessional). Adopt a rustic, natural tone.`
    },
    {
        id: 5,
        name: 'Luxury Menu Card',
        previewImages: ['https://images.unsplash.com/photo-1607921232004-92a061c0d5a3?q=80&w=1887&auto=format&fit=crop'],
        downloadFormats: { JPG: '#', PNG: '#' },
        category: 'Menu Cards',
        aiPrompt: `Create a sophisticated wedding menu for [FEMALE_NAME] and [MALE_NAME]'s reception on [WEDDING_DATE]. Suggest a starter, main course (vegetarian and non-vegetarian options), and dessert. Ensure it sounds exquisite.`
    },
    {
        id: 6,
        name: 'Minimalist Seating Plan',
        previewImages: ['https://images.unsplash.com/photo-1551893128-213b10860475?q=80&w=1887&auto=format&fit=crop'],
        downloadFormats: { PSD: '#', EPS: '#' },
        category: 'Seating Plans',
        aiPrompt: `Provide an example seating arrangement for [FEMALE_NAME] and [MALE_NAME]'s wedding reception on [WEDDING_DATE] at [ADDRESS], suggesting a head table and a few guest tables with names. Optimize for clear readability.`
    },
    {
        id: 7,
        name: 'Watercolor Invitation Suite',
        previewImages: ['https://images.unsplash.com/photo-1494809610214-f42a22be22b5?q=80&w=1887&auto=format&fit=crop'],
        downloadFormats: { PSD: '#', JPG: '#' },
        category: 'Invitations',
        aiPrompt: `Draft a modern, watercolor-themed wedding invitation for [FEMALE_NAME] and [MALE_NAME]'s wedding on [WEDDING_DATE] at [ADDRESS]. Emphasize an artistic and fluid style in the wording.`
    },
    {
        id: 8,
        name: 'Vintage Save-the-Date Postcard',
        previewImages: ['https://images.unsplash.com/photo-1530053335839-a35af3d7a8d5?q=80&w=1887&auto=format&fit=crop'],
        downloadFormats: { PNG: '#', JPG: '#' },
        category: 'Save-the-Date',
        aiPrompt: `Write a charming, vintage-style save-the-date postcard message for [FEMALE_NAME] and [MALE_NAME]'s wedding on [WEDDING_DATE]. Keep it sweet and nostalgic.`
    },
];
...
function TemplateCard({ template, onOpenModal }) {
    return (
        <div className="bg-white rounded-xl shadow-md overflow-hidden group relative transform transition-transform duration-300 hover:-translate-y-2">
            {/* इमेज के लिए 1:1 अनुपात वाला कंटेनर */}
...
function Modal({ isOpen, onClose, template, mode, setToastMessage }) {
    ...
    useEffect(() => {
        if (!isOpen) {
            // मोडल बंद होने पर स्टेट रीसेट करें
            setMaleName('');
            setFemaleName('');
            setWeddingDate('');
            setAddress('');
    ...
    const handleGenerateWithAI = async () => {
        ...
        try {
            let personalizedPrompt = template.aiPrompt
                .replace(/\[MALE_NAME\]/g, maleName)
                .replace(/\[FEMALE_NAME\]/g, femaleName)
                        ...
                        {Object.entries(template.downloadFormats).map(([format, link]) => (
                             link && (
                                <a
                                    ...
                                    onClick={onClose}
                                    className="block w-full text-center bg-violet-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-violet-700 transition-all duration-300"
                                >
                                    Download .{format}
                                </a>
                            )
...
// टोस्ट नोटिफ़िकेशन कंपोनेंट
const Toast = ({ message, onClose }) => {
    useEffect(() => {
        const timer = setTimeout(() => {
            onClose();
        }, 3000); // 3 सेकंड के बाद अपने आप बंद हो जाएगा
...
function App() {
    ...
    const [modalMode, setModalMode] = useState('download'); // 'download' या 'ai'
                    ...
                    <div className="flex flex-wrap justify-center gap-3 mb-10">
                        {categories.map((category) => (
                            <button
                                ...
                                className={`px-5 py-2 rounded-full font-semibold text-base transition-all duration-300 shadow-sm
                                ${selectedCategory === category
                                    ? 'bg-violet-600 text-white ring-2 ring-offset-2 ring-violet-600'
